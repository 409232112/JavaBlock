...
public class MsgHandleUtil {
    ...
    public static void invHandle(byte[] data){
        ...
        if("block".equals(type)){
            ...
            byte[] txId = inventory.getItems().get(0);
            System.out.println("-----"+ServerUtil.getMempool());
            if(ServerUtil.getMempool().get(DataUtil.byte2Hex(txId))==null){
                ServerUtil.sendGetData(inventory.getAddrFrom(),"tx",txId);
            }
        }
    }
    public static void getDataHandle(byte[] data){
        ...
        if("tx".equals(type)){
            ...
            //ServerUtil.getMempool().remove(txID);
        }
    }
    ...
    public static void txHandle(byte[] data){
        ...
        if(KnownNodes.getKnownNodes().get(0).equals(ServerUtil.getNodeAddress())){
            ...
            //当挖矿节点内存中有两笔交易进行打包处理
            if(ServerUtil.getMempool().size()>= NetConstant.packageCount && ServerUtil.getMiningAddress().length()>0){
                try{
